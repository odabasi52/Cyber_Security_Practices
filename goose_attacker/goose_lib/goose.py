class GOOSE:
    def __init__(self, appId, gocbRef, timeAllowedToLive, datSet, goId, t, 
        ...
        self.appId = b"\x00" + appId.to_bytes(length=1)
        ...
        self.stNum = b"\x85\x01" + stNum.to_bytes(length=1)
        self.sqNum = b"\x86\x01" + sqNum.to_bytes(length=1)
        self.simulation = b"\x87\x01" + simulation.to_bytes(length=1)
        self.confRev = b"\x88\x01" + confRev.to_bytes(length=1)
        self.ndsCom = b"\x89\x01" + ndsCom.to_bytes(length=1)
        self.numDatSetEntries = b"\x8a\x01" + numDatSetEntries.to_bytes(length=1)
